...
if (
    ...
    && $method !== 'update'  && $method !== 'delete'  && $method !== 'paginate'  && $method !== 'verify'
...
function createRecord($conn, $email, $password)
{
    try {
        ...
        if ($result) {
            ...
            $roleName = 'user';
            $query0 = 'INSERT INTO "Profile" (id, email, password, token, "roleId") VALUES (:id, :email, :password, :token, 
            ...
            $statement0->bindParam(':roleName', $roleName);
            $result0 = $statement0->execute();

            // $result0 = $statement0->execute();
  
            // if (isset($_POST['refParrain']))  create_affiliate($id, $_POST['refParrain']);

            if ($result0) {
                // Vérifier si l'utilisateur existe déjà
                $query01 = 'SELECT * FROM "Profile" WHERE email = :email';
                ...
                if ($result01) {
                    ...
                    $name = $_POST["Name"];
                    $link = $_POST["Link"];
                    // $link = 'http://localhost:3000/fr/me/verify?token=' . $token . '&email=' . urlencode($email);
                    $link = $link . '/fr/me/verify?token=' . $token . '&email=' . urlencode($email);
                    // Préparation des données pour la requête POST $result['id']
                    $postData = array(
                        'Email' => $email,
                        'Name' => $link, // Supposons que 'name' est une colonne
                        'Link' => $link, // Changez le lien si nécessaire
                        'TemplateId' => 22
                    );

                    // Envoi de la requête POST vers SendMail.php
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL, "http://test.myreklam.fr/backend/SendMail.php");
                    ...
                    if (curl_errno($ch)) {
                        ...
                        if (isset($responseData['status']) && $responseData['status'] === "success") {
                            // echo json_encode(array("status" => "success", "message" => "Email envoyé avec succès"));
                            echo json_encode(array("status" => "success", "message" => "Connexion réussie et Email envoyé avec succès", "id" => $result01['id']));
                        } else {
                            echo json_encode(array("status" => "error", "message" => "Échec de l'envoi de l'email", "respone" => $responseData));
                        }
                    }
...
function forgotPassword($conn, $email)
{
    try {
        ...
        $queryUpdate = 'UPDATE "Profile" SET token = :reset_token WHERE email = :email';
...
function resetPassword($conn, $email, $newPassword, $token)
{
    try {
        ...
        $query = 'SELECT * FROM "Profile" WHERE email = :email AND token = :token';
        $statement = $conn->prepare($query);
        $statement->bindParam(':email', $email);
        ...
        $queryUpdate = 'UPDATE "Profile" SET password = :password, token = NULL WHERE email = :email';
        $statementUpdate = $conn->prepare($queryUpdate);
...
function verifyEmailAndToken($conn, $email, $token)
{
    try {
        ...
        $query = 'SELECT id FROM "Profile" WHERE email = :email AND token = :token';
        $statement = $conn->prepare($query);
        $statement->bindParam(':email', $email);
        $statement->bindParam(':token', $token);
        ...
        if ($result) {
            ...
            $updateQuery = 'UPDATE "Profile" SET token = :token, isVerified = :isVerified WHERE id = :id';
...
{
    // $sql = "SELECT * FROM UserConnect WHERE email = '".$email."'";
    ...
    if ($result) {
        ...
        if (password_verify($password, $result['password'])) {
            ...
            //         if (isset($responseData['status']) && $responseData['status'] === "success") {
            //             // echo json_encode(array("status" => "success", "message" => "Email envoyé avec succès"));
            //             echo json_encode(array("status" => "success","message" => "Connexion réussie et Email envoyé avec succès","id" => $result['id']));
            //         } else {
            //             echo json_encode(array("status" => "error", "message" => "Échec de l'envoi de l'email"));
            //         }
            //     }

            //     curl_close($ch);
            echo json_encode(array("status" => "success", "message" => "Connexion réussie 0", "isverified" => $result['isverified']));
        } else {
        ...
        // L'utilisateur n'existe pas dans la base de données result
        $response = array("status" => "error", "message" => "Utilisateur non trouvé");
        echo json_encode(array("status" => "error", "message" => "Utilisateur non trouvé", "message0" => $result->num_rows));
    }
    // echo json_encode(array("status" => "success","data" => $result));
    //     }else {
    //     http_response_code(500); // Erreur de serveur interne
    //     echo json_encode(array("status" => "failure", "message1" => "Failed to get record", "message0" => $result));
    // }


    // echo json_encode(array("status" => "failure", "message" => $result));
}
