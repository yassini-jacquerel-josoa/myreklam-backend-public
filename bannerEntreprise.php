...
function createActualite($conn) {
    ...
    $stmt = $conn->prepare('INSERT INTO userInfo (id, userId, title, url, urlPhoto) VALUES (:id, :userId, :title, :url, :urlPhoto)');
...
function readActualites($conn) {
    ...
    if ($userId) {
        $stmt = $conn->prepare('SELECT * FROM userInfo WHERE userId = :userId ORDER BY createdAt DESC');
        $stmt->bindParam(':userId', $userId);
        $stmt->execute();
    } else {
        $stmt = $conn->query('SELECT * FROM userInfo ORDER BY createdAt DESC');
    }
...
function deleteActualite($conn) {
    ...
    $stmt = $conn->prepare('DELETE FROM userInfo WHERE id = :id');
    $stmt->bindParam(':id', $id);
