...
function updateAd($conn) {
    try {
        ...
        $query = 'UPDATE "favoris" SET ' . implode(', ', $fieldsToUpdate) . ' WHERE "id" = :id';
        $statement = $conn->prepare($query);
    ...
    function readAds($conn) {
        try {
            ...
            $query = 'SELECT * FROM "favoris" ORDER BY "id" DESC';
            $statement = $conn->prepare($query);
            ...
            if ($result) {
                echo json_encode([
                    "status" => "success",
                    "favoris" => $result // Inclure les données récupérées dans la réponse
                ]);
            } else {
            ...
            echo json_encode([
                "status" => "error",
                "message" => "An error occurred while fetching the favoris.",
                "details" => $e->getMessage()
            ]);
        }
    }
...
//                 $query = 'SELECT * FROM favoris WHERE 1=1'; // Base de la requête
//                 $params = [];
...
//                 setJsonHeader();
//                 if ($result) {
//                     echo json_encode(["status" => "success", "favoris" => $result]);
//                 } else {
//                     http_response_code(404); // Aucun résultat
//                     echo json_encode(["status" => "failure", "message" => "No favoris found matching the criteria."]);
//                 }
//             } catch (Exception $e) {
//                 setJsonHeader();
...
function readAdsByCriteria($conn) {
    try {
        ...
        if ($result) {
            ...
            // http_response_code(404); // Aucun résultat
            // echo json_encode(["status" => "failure", "message" => "No favoris found matching the criteria.", "favoris" => [] ]);
            echo json_encode(["status" => "success", "favoris" => [] ]);
        }
    } catch (Exception $e) {
        setJsonHeader();
        http_response_code(500); // Erreur interne
    ...
    function deleteRecord($conn, $userid, $annonceid) {


        if ($userid !== null && $annonceid !== null) {
            $query = "DELETE FROM \"favoris\" WHERE \"userid\" = :userid AND \"annonceid\" = :annonceid";
            $statement = $conn->prepare($query);
            ...
            http_response_code(400); // Mauvaise requête
            echo json_encode(array("status" => "failure", "message" => "userid and annonceid are required"));
        }
    }
...
logToFile("Fichiers reçus : " . json_encode($_FILES));
    ...
    if (isset($_FILES['photoProfil']) && $_FILES['photoProfil']['error'] === UPLOAD_ERR_OK) {
        ...
        deleteRecord($conn, $userid, $annonceid);
