...
function updateAd($conn) {
    try {
        ...
        $query = 'UPDATE "ads" SET ' . implode(', ', $fieldsToUpdate) . ' WHERE "id" = :id';
        $statement = $conn->prepare($query);
...
//                 $query = 'SELECT * FROM ads WHERE 1=1'; // Base de la requête
//                 $params = [];
...
//                     echo json_encode(["status" => "failure", "message" => "No ads found matching the criteria."]);
//                 }
//             } catch (Exception $e) {
//                 setJsonHeader();
